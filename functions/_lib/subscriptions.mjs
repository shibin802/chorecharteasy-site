...
export async function subscribe(request, env, h) {
  ...
  if (customer.checkout_url && customer.checkout_expires_at > now && cachedAttempt?.price_id === price.id && !cachedAttempt.id.startsWith('usd-') && cachedAttempt.checkout_id === customer.checkout_id) {
