import { stripeClient, isLiveBilling } from './billing.mjs';
...
export function eligibleInvoice(invoice, customerId, live = false) {
  return Boolean(customerId && invoice.customer === customerId && invoice.livemode === live && invoice.status === 'paid' && invoice.amount_paid > 0);
}
export async function billingHistory(request, env, h) {
  ...
  return h.jsonResponse({ok:true,testMode:!isLiveBilling(env),hasMore:invoices.has_more,requests:requests.results,invoices:invoices.data.filter(i=>i.customer===customerId && i.livemode === isLiveBilling(env)).map(i=>({id:i.id,number:i.number,amount:i.amount_paid,currency:i.currency,status:i.status,created:i.created,canRequest:eligibleInvoice(i,customerId,isLiveBilling(env))}))});
}
export async function requestRefund(request, env, h) {
  ...
  if(!eligibleInvoice(invoice,customerId,isLiveBilling(env))) throw new h.ApiError(404,'payment_not_found','An eligible paid invoice was not found.');
  ...
  return h.jsonResponse({ok:true,testMode:!isLiveBilling(env),request:saved});
}
