...
class FrontendV2Contract(unittest.TestCase):
    ...
    def test_plausible_is_enabled_except_on_private_account_page(self):
    ...
    def test_shared_assets_and_security_headers(self):
        home = self.text("index.html")
        self.assertIn('href="/assets/site.css?v=20260919-print-action"', home)
        self.assertIn('src="/assets/consent.js?v=20260823-consent-copy-v2"', home)
        self.assertIn('src="/assets/site.js?v=20260919-pdf-title-v2"', home)
    ...
    def test_mobile_layout_guards_common_393px_iphones(self):
        ...
        self.assertRegex(css, re.compile(r"@media\(max-width:480px\).*?\.table-wrap\{overflow-x:hidden", re.S))
        self.assertRegex(css, re.compile(r"@media\(max-width:480px\).*?\.chart-table\{min-width:0;width:100%;table-layout:fixed", re.S))
        self.assertRegex(css, re.compile(r"@media\(max-width:480px\).*?\.swipe-hint\{display:none", re.S))
    ...
    def test_mutable_assets_are_revalidated_and_css_is_cache_busted(self):
        ...
        for path in ROOT.glob("*.html"):
            ...
            if "/assets/site.css" in html:
                site_css_version = "20260919-print-action" if path.name == "index.html" else "20260822-consent-bar-v1"
            ...
            if "/assets/site.js" in html:
                site_js_version = "20260919-pdf-title-v2" if path.name == "index.html" else "20260822-ux-v1"
    ...
    def test_sitewide_feedback_is_real_minimized_and_disclosed(self):
        ...
        for page in public_pages:
            ...
            feedback_css_version = "20260919-feedback-v2"
            self.assertIn(f"/assets/feedback.css?v={feedback_css_version}", html, page.name)
            self.assertIn("/assets/feedback.js?v=20260919-feedback-v2", html, page.name)
